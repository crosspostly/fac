...
def _set_description(driver: WebDriver, description: str) -> None:
    ...
    # Remove any characters outside the BMP range (emojis, etc) for send_keys compatibility
    ...
    def remove_overlays(driver):
        try:
            script = """
            document.querySelectorAll('.TUXModal-overlay, .react-joyride__overlay, [data-test-id="overlay"]').forEach(el => el.remove());
            """
    ...
    # Robust click with overlay removal
    try:
        desc.click()
    except ElementClickInterceptedException:
        logger.debug(green("Click intercepted, removing overlays and retrying..."))
        remove_overlays(driver)
        time.sleep(1)
        try:
            desc.click()
        except ElementClickInterceptedException:
            logger.debug(green("Click intercepted again, forcing click with JS..."))
...
def _remove_cookies_window(driver) -> None:
    ...
    try:
        ...
        except (NoSuchShadowRootException, Exception):
            driver.execute_script(
                "try { document.querySelector(arguments[0]).remove(); } catch (e) {}",
                config.selectors.upload.cookies_banner.banner,
            )
    except Exception as e:
        logger.debug(red(f"Error removing cookies window (ignored): {e}"))
    ...
    **kwargs,
    ...
    try:
        ...
        try:
            ...
            logger.debug(red("Comment checkbox not found, skipping..."))
        ...
        try:
            ...
            logger.debug(red("Stitch checkbox not found, skipping..."))
        ...
        try:
            ...
            logger.debug(red("Duet checkbox not found, skipping..."))
